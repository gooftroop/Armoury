...
| FE-051 | Local component state must be the preferred default for UI state (open/closed, hover, focus, scroll position) using `useState` or `useReducer` | Critical | Simplest option first |
...
| FE-058 | `useReducer` must be preferred over `useState` when 3+ interrelated state values exist or when complex state transitions (state machine) are involved | High | Structured state updates |
