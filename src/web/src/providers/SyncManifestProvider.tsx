 ...
 */
const SyncManifestReactContext = createContext<SyncManifestValue | undefined>(undefined);
 ...
 */
export function SyncManifestProvider({ children }: SyncManifestProviderProps): ReactElement {
     ...
     * @returns Nothing.
     */
    const markSynced = useCallback((systemId: string): void => {
        setSyncedSet((prev) => {
    ...
    const syncedSystems = useMemo<string[]>(() => Array.from(syncedSet), [syncedSet]);
    const value = useMemo<SyncManifestValue>(
        () => ({
            markSynced,
            hasSynced,
            syncedSystems,
...
export function useSyncManifest(): SyncManifestValue {
    const context = useContext(SyncManifestReactContext);
