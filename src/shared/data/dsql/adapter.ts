...
import { getEntityCodec } from '@shared/data/codec.js';
import type { FileSyncStatus } from '@shared/types/entities.js';
import { DatabaseError } from '@shared/types/errors.js';
import { syncStatus } from '@shared/data/dsql/schema.js';
import { getMergedDSQLSchema } from '@shared/data/schema.js';
...
const { DsqlSigner } = require('@aws-sdk/dsql-signer') as { DsqlSigner: DsqlSignerConstructor };
const { Client } = require('pg') as { Client: PgClientConstructor };
const { drizzle } = require('drizzle-orm/node-postgres') as { drizzle: DrizzleFactory };
const {
    eq,
    asc: drizzleAsc,
    desc: drizzleDesc,
} = require('drizzle-orm') as {
    eq: EqBuilder;
    asc: OrderByFn;
    desc: OrderByFn;
};
...
export class DSQLAdapter extends BaseDatabaseAdapter {
    ...
    async getSyncStatus(fileKey: string): Promise<FileSyncStatus | null> {
        ...
        try {
            const results = await db.select().from(syncStatus).where(eq(syncStatus.fileKey, fileKey)).limit(1);
    ...
    async deleteSyncStatus(fileKey: string): Promise<void> {
        ...
        try {
            await db.delete(syncStatus).where(eq(syncStatus.fileKey, fileKey));
        } catch (error) {
            throw new DatabaseError(
