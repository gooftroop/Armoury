...
/**
 * @requirements
 * - REQ-DI-060: Matches Lambda handler resolves production DatabaseAdapter from DI container.
...
describe('matches handler', () => {
    ...
    it('logs invocation and completion via Sentry logger', async () => {
        ...
        expect(mocks.loggerInfo).toHaveBeenNthCalledWith(1, '[matches] Handler invoked', {
            httpMethod: 'GET',
            path: '/matches',
        });
        expect(mocks.loggerInfo).toHaveBeenNthCalledWith(2, '[matches] Handler completed', {
            httpMethod: 'GET',
            path: '/matches',
            statusCode: 200,
        });
