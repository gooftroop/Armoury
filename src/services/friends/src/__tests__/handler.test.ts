...
/**
 * @requirements
 * - REQ-DI-060: Friends Lambda handler resolves production DatabaseAdapter from DI container.
...
describe('friends handler', () => {
    ...
    it('logs invocation and completion via Sentry logger', async () => {
        ...
        expect(mocks.loggerInfo).toHaveBeenNthCalledWith(1, '[friends] Handler invoked', {
            httpMethod: 'GET',
            path: '/friends',
        });
        expect(mocks.loggerInfo).toHaveBeenNthCalledWith(2, '[friends] Handler completed', {
            httpMethod: 'GET',
            path: '/friends',
            statusCode: 200,
        });
