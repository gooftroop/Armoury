/**
 * Structured WebSocket error capture for the friends service.
 *
 ...
 */
export type WsErrorSource = 'adapter:init' | 'auth:extract' | 'broadcast:send' | 'db:operation' | 'route:not-found';
...
export interface WsErrorContext {
    ...
    /** WebSocket route key ($connect, $disconnect, $default). */
    routeKey?: string;

    /** Authenticated user ID, when available. */
    userId?: string;

    /** Additional context fields (e.g. operation name, store). */
    [key: string]: unknown;
}
